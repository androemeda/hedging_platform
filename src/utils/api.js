...
const BASE_URL =
  import.meta.env.MODE === 'development'
    ...
    : 'https://king-prawn-app-td6jt.ondigitalocean.app/api'; // production backend URL
...
export const api = {
  ...
  getActiveContracts: async (userType, userId = null) => {
    try {
      const endpoint =
        userType === 'farmer'
          ? '/farmer/active-contracts'
          : '/trader/active-contracts';
      const params = { email: getEmail() };
  ...
  getPendingContracts: async (userType, userId = null) => {
    try {
      const endpoint =
        userType === 'farmer'
          ? '/farmer/pending-contracts'
          : '/trader/pending-contracts';
      const params = { email: getEmail() };
      if (userId) params[`${userType}_id`] = userId;
      const { data } = await axiosInstance.get(endpoint, { params });
      return data;
  ...
  getCurrentPrices: async (type = null) => {
    try {
      ...
      const { data } = await axiosInstance.get('/market/current-prices', {
        params,
      });
      return data;
    } catch (error) {
  ...
  getFarmerDashboard: async (farmerId = null) => {
    try {
      const params = { email: getEmail() };
      if (farmerId) params.farmer_id = farmerId;
      const { data } = await axiosInstance.get('/farmer/dashboard-summary', {
        params,
      });
      return data;
    } catch (error) {
  ...
  getTraderDashboard: async (traderId = null) => {
    try {
      ...
      const { data } = await axiosInstance.get('/trader/dashboard-summary', {
        params,
      });
      return data;
    } catch (error) {
